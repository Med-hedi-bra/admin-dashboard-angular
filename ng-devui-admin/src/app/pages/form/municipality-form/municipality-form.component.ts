import { Component } from '@angular/core';
import { FormLayout } from 'ng-devui';
import { Observable, of } from 'rxjs';
import { delay } from 'rxjs/operators';
import { ListDataService } from 'src/app/@core/mock/list-data.service';
@Component({
  selector: 'app-municipality-form',
  templateUrl: './municipality-form.component.html',
  styleUrls: ['./municipality-form.component.scss']
})
export class MunicipalityFormComponent {

  status = '';
  projectFormData = {
    projectName: '',
    projectOwner: null,
    projectExecutor: null,
    projectCycleTime: [null, null],
    projectSecurity: 'Only member visible',
    projectDescription: '',
    projectExerciseDate: [{ id: '1', label: 'Mon' }],
  };

  municipalityFormData = {
    idMun: '',
    president: '',
    secritaire: '',
    maitre: ''
  }

  verticalLayout: FormLayout = FormLayout.Vertical;

  existprojectNames = ['123', '123456', 'DevUI'];

  checkboxOptions = [
    { id: '1', label: 'Mon' },
    { id: '2', label: 'Tue' },
    { id: '3', label: 'Wed' },
    { id: '4', label: 'Thur' },
    { id: '5', label: 'Fri' },
    { id: '6', label: 'Sat' },
  ];

  securityValue = ['Public', 'Only member visible'];

  OwnerOptions = [
    { id: '1', name: 'Owner1' },
    { id: '2', name: 'Owner2' },
    { id: '3', name: 'Owner3' },
    { id: '4', name: 'Owner4' },
  ];

  ExecutorOptions = [
    { id: '1', name: 'Executor1' },
    { id: '2', name: 'Executor2' },
    { id: '3', name: 'Executor3' },
    { id: '4', name: 'Executor4' },
  ];
  constructor(private listDataService: ListDataService) { }
  getValue(value: object) {
    console.log(value);
  }

  everyRange(range: any) {
    return range.every((_: any) => !!_);
  }

  checkCodePostal(value: string) {
    let res = true;
    let code = parseInt(this.municipalityFormData.idMun)
    if (isNaN(code)) {
      res = false;
    }
    return of(res).pipe(delay(500));
  }

  validateDate(value: any): Observable<any | null> {
    let message = null;
    for (const item of value) {
      if (item.id === '2') {
        message = {
          'zh-cn': `当前日期队列已满`,
          'en-us': 'The task queue on the current execution day (Tuesday) is full.',
        };
      }
    }
    // Returned by the simulated backend interface
    return of(message).pipe(delay(300));
  }

  submitProjectForm({ valid, directive, data, errors }: any) {
    if (valid) {
      // do something
      let test = this.listDataService.addNewMunicipality(this.municipalityFormData);
      console.log(test)
      this.status = 'success';

    } else {
      // error tip
      this.status = 'failure'
    }
  }



}
